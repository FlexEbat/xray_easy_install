...
这是一个通用且高度自动化的 Bash 脚本，用于部署基于 **Xray-core** 的专业级 VPN 服务器。该脚本专为绕过深度数据包检测系统（如俄罗斯的 DPI TCPY）和现代防火墙而设计，采用了先进的 **VLESS-REALITY** 和 **XHTTP** 协议。
...
本项目仅出于教育和研究目的开发，旨在研究最小化数字足迹和保护在线用户隐私的方法。该脚本用于测试绕过跨区域封锁及深度数据包检测（DPI）系统的方法，并按“现状”（AS IS）提供。作者对任何滥用本工具的行为、可能导致的资源封锁或违反当地法律法规的情况不承担任何责任。您使用此代码需自行承担风险，并对操作产生的所有后果负全部责任。
