...
generate_xray_config() {
    ...
    while IFS=":" read -r U_NAME U_UUID U_SHORT U_SECRET || [[ -n "$U_NAME" ]]; do
        ...
        VISION_CLIENTS+="$(printf '{"id":"%s","flow":"xtls-rprx-vision"},' "$U_UUID")"
        WS_CLIENTS+="$(printf '{"id":"%s"},' "$U_UUID")"
    ...
    # Добавляем сокет Nginx только если используется режим своего домена (3 пункт)
...
generate_user_pages() {
    ...
    while IFS=":" read -r U_NAME U_UUID U_SHORT U_SECRET || [[ -n "$U_NAME" ]]; do
        if [[ -z "$U_NAME" ]]; then continue; fi
        LINK_VISION="vless://${U_UUID}@${SERVER_IP}:443?type=tcp&security=reality&pbk=${PUBLIC_KEY}&fp=chrome&sni=${REALITY_DEST}&sid=${U_SHORT}&spx=%2F&flow=xtls-rprx-vision#${U_NAME}-Vision"
        LINK_XHTTP="vless://${U_UUID}@${SERVER_IP}:8443?type=xhttp&security=reality&pbk=${PUBLIC_KEY}&fp=chrome&sni=${REALITY_DEST}&sid=${U_SHORT}&path=${XHTTP_PATH}#${U_NAME}-XHTTP"

        if [[ "$HAS_DOMAIN" == "true" ]]; then
            LINK_WS="vless://${U_UUID}@${DOMAIN}:2053?type=xhttp&security=tls&path=${NGINX_WS_PATH}#${U_NAME}-XHTTP-CDN"
            RAW_SUB="${LINK_VISION}\n${LINK_XHTTP}\n${LINK_WS}"
...
install_core() {
    ...
    if [[ "$SETUP_MODE" == "1" ]]; then
        HAS_DOMAIN=false
        read -p "Введите сайт для маскировки (например, www.apple.com): " REALITY_DEST
        ...
        read -p "Скопируйте и введите выбранный сайт из списка выше: " REALITY_DEST
        rm -f RealiTLScanner
        ...
        if [[ "$REAL_TYPE" == "1" ]]; then
            read -p "Введите внешний сайт для REALITY (например, www.samsung.com): " REALITY_DEST
        else
            REALITY_DEST="$DOMAIN"
    ...
    if [[ -n "$NEW_SSH_PORT" ]]; then
        sed -i -e "s/^#Port .*/Port $NEW_SSH_PORT/" -e "s/^Port .*/Port $NEW_SSH_PORT/" /etc/ssh/sshd_config
    ...
    if [[ "$HAS_DOMAIN" == "true" ]]; then
        apt install -y -q jq nginx certbot python3-certbot-nginx uuid-runtime gnupg coreutils fail2ban ufw cron
    else
        apt install -y -q jq uuid-runtime gnupg coreutils fail2ban ufw cron
    fi
...
EOF
    ...
    # ВЫВОДИМ ССЫЛКИ СРАЗУ ПОСЛЕ УСТАНОВКИ
    ...
    while IFS=":" read -r U_NAME U_UUID U_SHORT U_SECRET || [[ -n "$U_NAME" ]]; do
        ...
        if [[ "$HAS_DOMAIN" == "true" ]]; then
            ...
            LINK_VISION="vless://${U_UUID}@${SERVER_IP}:443?type=tcp&security=reality&pbk=${PUBLIC_KEY}&fp=chrome&sni=${REALITY_DEST}&sid=${U_SHORT}&spx=%2F&flow=xtls-rprx-vision#${U_NAME}-Vision"
            LINK_XHTTP="vless://${U_UUID}@${SERVER_IP}:8443?type=xhttp&security=reality&pbk=${PUBLIC_KEY}&fp=chrome&sni=${REALITY_DEST}&sid=${U_SHORT}&path=${XHTTP_PATH}#${U_NAME}-XHTTP"
            echo -e "Vision (Порт 443): ${CYAN}$LINK_VISION${NC}\n"
            echo -e "XHTTP (Порт 8443): ${CYAN}$LINK_XHTTP${NC}"
        fi
    done < "$USERS_FILE"
    ...
    echo -e "\n${YELLOW}=======================================${NC}"
    echo -e "${YELLOW}Нажмите ENTER, чтобы открыть главное меню...${NC}"
    read -r
}
...
uninstall_xray() {
    ...
    if [[ "$CONFIRM" == "y" ]]; then
        ...
        if command -v warp-cli &> /dev/null; then warp-cli --accept-tos disconnect 2>/dev/null || true; fi
...
while true; do
    ...
    if [[ ! -f "$CONF_FILE" ]]; then
        ...
        case $MENU_CHOICE in
            ...
            2) manage_warp ;;
            3) toggle_warp ;;
            4) generate_xray_config ; generate_user_pages; echo -e "${GREEN}Пересоздано!${NC}" ; sleep 1 ;;
